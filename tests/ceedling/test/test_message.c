...
void test_keymap_serialise_event_press(void) {
    ...
    KeymapInputEvent event = { .event_type = KeymapEventPress, .value = 4, };
...
void test_keymap_deserialise_event_press(void) {
    KeymapInputEvent expected_event = { .event_type = KeymapEventPress, .value = 4, };
    ...
    TEST_ASSERT_EQUAL_MEMORY(&expected_event, &actual_event, sizeof(KeymapInputEvent));
}

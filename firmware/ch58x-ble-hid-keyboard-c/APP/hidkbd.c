...
uint16_t HidEmu_ProcessEvent(uint8_t task_id, uint16_t events) {
  ...
  if (events & KEYBOARD_SCAN_EVT) {
    ...
    for (uint16_t i = 0; i < KEYBOARD_MATRIX_KEY_COUNT; i++) {
      if (new_states[i] != 0) {
        ...
        uint32_t delta_time_tmos =
            current_time_tmos - prev_keymap_updated_time_tmos;
    ...
    tmos_start_task(hidEmuTaskId, KEYMAP_TICK_EVT,
                    KEYMAP_POLLING_INTERVAL_TMOS);
  ...
  if (events & KEYMAP_TICK_EVT) {
    ...
    // If the keymap still requires polling, reschedule this event.
    // Otherwise, the system will eventually go back to interrupt-based waiting
    // once KEYBOARD_SCAN_EVT sees no pressed keys and keymap_requires_polling()
    // is false.
    if (keymap_requires_polling() || keyboard_matrix_pressed_keys_count() > 0) {
      tmos_start_task(hidEmuTaskId, KEYMAP_TICK_EVT,
                      KEYMAP_POLLING_INTERVAL_TMOS);
    } else {

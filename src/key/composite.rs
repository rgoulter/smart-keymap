//! Full composite aggregation of all [crate::key::System] implementations.
//!
//! This module is the **complete** key-system shell used by cucumber,
//!  the ! `keymap!` macro,
//!  and non-custom builds.
//! Firmware builds with a custom keymap instead
//!  get a **trimmed** per-keymap `init::key_system` module generated by Nickel
//!  (`ncl/composite-key-system.ncl`)
//!  that only includes the families that keymap uses
//!  (e.g. keyboard + tap-hold + layered).
//!
//! The generated module is intentionally human-readable
//!  and mirrors the structure here:
//!  `Ref` / `Event` / `PendingKeyState` / `KeyState` enums,
//!  `Config` / `Context` / `System`,
//!  and a `key::System<Ref>` impl.
//!
//! When adding a new key family, update both:
//! 1. this full composite (enums, bridges, dispatch), and
//! 2. the family registry in `ncl/composite-key-system.ncl`.
